...
searchAux f node graph structura viz = if null structura then [] 
                                       else 
                                           head structura : searchAux f 
                                                                        (head (tail (f structura [x|x <- (S.toList(outNeighbors node graph)), not (x `elem` viz)] ) )) 
                                                                        graph 
                                                                        (tail (f structura [x|x <- (S.toList(outNeighbors node graph)), not (x `elem` viz)] ) )
                                                                        (S.toList(S.fromList(viz ++ [node]))) 
...
bfs = search (\x y -> removeDuplicatesLeft(x ++ y)) 
...
dfs = search (\x y -> removeDuplicatesLeft([head x] ++ y ++ (tail x))) 
...
countBFS :: Ord a
            ...
            -> StandardGraph a
            -> Int
countBFS sursa dest graph = let bfsResult = bfs sursa graph
                                hasWayToDest = if dest `elem` bfsResult then True else False
                            in 
                                if hasWayToDest then (length(fst(span (/= dest) bfsResult)) - 1) else 0
...
countDFS :: Ord a
            ...
            -> StandardGraph a
            -> Int
countDFS sursa dest graph = let dfsResult = dfs sursa graph
                                hasWayToDest = if dest `elem` dfsResult then True else False
                            in 
                                if hasWayToDest then (length(fst(span (/= dest) dfsResult)) - 1) else 0
...
countIntermediate from to graph = if ((countBFS from to graph) == 0) || ((countDFS from to graph) == 0) then Nothing
                                  else Just((countBFS from to graph), (countDFS from to graph))
